...
Modify the roadmap structure after brainstorming revisions, scope changes, new features, or course corrections. Unlike `update-task` (which handles individual status changes), this skill handles structural changes to the roadmap itself.
...
2. For each new task, confirm:
   - Title
   - Complexity (standard or complex)
   - Context, objective, acceptance criteria, scope
   - Technical approach and test plan (if complex)
   - Dependencies (which existing tasks must complete first)
   - Phase placement
3. Create individual task files using the appropriate template from the plugin's root `templates/` directory (not the skill's own folder).
4. Update MILESTONE.md: add task IDs to the milestone's task list.
5. Update TASK.md: add task rows to the appropriate phase table. If a new phase is needed, create it.
6. Update ROADMAP.md: update progress column for the affected milestone.

### Cancel Milestone

1. Confirm with the user: "Cancel milestone **<name>** and all its tasks? This sets status to `cancelled` — files are preserved."
2. Set all task files in the milestone directory to `status: cancelled`.
...
2. For each task: follow the cascade update logic from `update-task` skill, setting status to `cancelled`.
...
4. Inform user: "Milestone **<name>** status overridden to **<status>**. Derived status would be **<derived>**. Override is visible in MILESTONE.md and ROADMAP.md."
...
> Updated roadmap for **<project-name>**:
> - Added milestone: **<name>** (N tasks)
> - Cancelled tasks: <list>
> - Phase 2 reordered: <summary>
>
> Current state: N milestones, M total tasks (X completed, Y in_progress, Z pending)
